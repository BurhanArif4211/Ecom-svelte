...
<script>
	...
	onMount(async () => {
		try {
			...
			if (
				...
				const selectedRate =
					firstPackage.shipping_rates.find(
						(rate) => rate.selected,
					);
				if (selectedRate) {
					selectedShipping = selectedRate.rate_id;
				} else if (
					firstPackage.shipping_rates.length > 0
				) {
					// Auto-select free shipping or first available
					const freeShipping =
						firstPackage.shipping_rates.find(
							(rate) =>
								parseFloat(
	...
	const updateCustomerInfo = async () => {
		try {
			...
			// Auto-select free shipping after address update
			if (
				$cart.shipping_rates &&
				$cart.shipping_rates.length > 0
			) {
				...
				if (
					...
					selectedShipping = freeShipping.rate_id;
					await selectShipping(
						freeShipping.rate_id,
					);
				}
	...
	const placeOrderHandler = async () => {
		try {
			...
			const data = await placeOrder({
				...
				shipping_address: useSameAddress
					? { ...billing, country: "PK" }
					: { ...shipping, country: "PK" },
			});
	...
	const getShippingRates = () => {
		if (!$cart.shipping_rates || $cart.shipping_rates.length === 0)
			return [];
		return $cart.shipping_rates[0].shipping_rates || [];
	};

	let rates = getShippingRates();
</script>
...
<div class="min-h-screen bg-black text-white py-8">
	<div class="container mx-auto px-4">
		{#if orderPlaced}
			<!-- Order Confirmation -->
			<div
				class="max-w-3xl mx-auto bg-gray-900 rounded-xl shadow-lg overflow-hidden border border-gray-800"
			>
				<div class="p-8 text-center">
					<div
						class="w-20 h-20 bg-green-900 rounded-full flex items-center justify-center mx-auto mb-6"
					>
						<svg
					...
					<div
						class="bg-gray-800 rounded-lg p-6 mb-8 text-left border border-gray-700"
					>
						<h2
						...
						<div class="mt-6">
							<h3
								class="font-medium mb-2 text-white"
							>
								Shipping Address
							</h3>
							<p
							...
							>
								...
								/>
								{#if billing.address_2}{billing.address_2}<br
									/>{/if}
			...
			<div class="max-w-6xl mx-auto">
				...
				>
					...
					<div class="lg:col-span-2 space-y-8">
						<!-- Billing Address -->
						<div
							class="bg-gray-900 rounded-xl shadow-lg p-6 border border-gray-800"
						>
							<h2
							...
							{#if error}
								<div
									class="bg-red-900 text-red-200 p-4 rounded-lg mb-6 border border-red-800"
								>
									{error}
								</div>
							{/if}
							...
							>
								...
								{#if rates.length > 0}
									...
									>
										{#each rates as rate}
											<div
												class:selected={selectedShipping ===
													rate.rate_id}
											>
												<input
													type="radio"
												...
												>
													...
													>
														...
														<span
														>
															{#if parseFloat(rate.price) === 0}{:else}
																{formatPrice(
																	rate.price,
																)}
															{/if}
					...
					<div>
						<div
							class="bg-gray-900 rounded-xl shadow-lg p-6 border border-gray-800 sticky top-8"
						>
							<h2
								class="text-xl font-bold mb-6 text-white"
							>
							...
							>
								...
								{#if $cart.needs_shipping}
									<div>
										...
										>
											{#if selectedShipping}
												{#if parseFloat($cart.totals?.total_shipping) === 0}
													Free
												{:else}
													{formatPrice(
														$cart
								...
								<div
									class="flex justify-between border-t border-gray-700 pt-3 font-bold text-lg"
								>
									<span
										class="text-white"
										>Total</span
							...
							<!-- Payment Method Info -->
							<div
								class="bg-blue-900 bg-opacity-20 border border-blue-800 rounded-lg p-4 mb-6"
							>
								<div
									class="flex items-center"
								>
